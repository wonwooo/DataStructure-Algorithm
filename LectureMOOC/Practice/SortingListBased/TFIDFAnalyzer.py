...
import csv
import math
import SortingListBased.SortingAlgorithm
import SortingListBased.BagOfWordCreator as BagOfWordCreator
sys.path.append('C:/Users/a2678/Desktop/DataStructure')
...
class TFIDFAnalyzer:
    ...
    def calculateTermFrequency(self): #use self.bows, self.words
        termFrequencies = []
        retWords = []
    ...
    def calculateDocumentFrequency(self): #use self.bows, self.words
        documentFrequencies = []
        retWords = []
        # We have stored a list of words in "self.words"
    ...
    def calculateTFIDF(self): #use two methods above
        TFIDF = []
        ...
        for itr in range(len(self.words)):
            ...
            TFIDF.append(termFrequencies[itr]*math.log(len(self.bows)/documentFrequencies[itr]))
        return retWords, TFIDF

if __name__ == "__main__":
    analyzer = TFIDFAnalyzer('./SortingListBased/talk.politics.misc', 10)
